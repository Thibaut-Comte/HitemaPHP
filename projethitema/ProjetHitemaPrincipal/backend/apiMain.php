...
if(isset($_SESSION['username']))
{
		function get_list_articles($base) {
			...
			array_push($Liste_article, array("ID" => $dataBase['ID'], "Localisation" => $dataBase['Localisation'], "Name" => $dataBase['Name'], "Longitude" => $dataBase['longitude'], "Latitude"=> $dataBase['latitude'], "Image" => $dataBase['Image']));
		...
		function get_article_by_Localisation($Localisation, $base) { //je passe en paramètre de ma fonction l'id de l'article souhaité et l'objet PDO pour exécuter la requête
			...
			{
				array_push($Detail_article, array("ID" => $result['ID'], "Localisation" => $result['Localisation'], "Name" => $result['Name'], "Longitude" => $result['longitude'], "Latitude"=> $result['latitude'], "Image" => $result['Image']));//je mets le résultat de ma requête dans une variable
			}
